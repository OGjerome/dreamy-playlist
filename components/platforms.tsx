...
export default function Platforms() {
  ...
  // Déterminer dynamiquement quelles plateformes sont connectées
  // Pour l'instant, seul Spotify est supporté via la session OAuth
  const isSpotifyConnected = !!session?.accessToken;

  const isPlatformConnected = (platformId: string): boolean => {
    if (platformId === "spotify") return isSpotifyConnected;
    // Les autres plateformes ne sont pas encore implémentées
    return false;
  };

  const connectedCount = platformConfigs.filter((p) =>
                ...
                <div className="space-y-3">
                  <div className="flex justify-between">
                    ...
                    <span className="text-white font-semibold">
                      {connectedCount}/{platformConfigs.length}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-400">Statut</span>
                    <span className="text-white font-semibold">
                      {isSpotifyConnected ? "Spotify actif" : "Aucune connexion"}
                    </span>
                  </div>
                </div>
              </CardContent>
              ...
              <CardContent className="space-y-3">
                {platformConfigs.map((platform) => (
                  <Button
                    key={platform.id}
                    variant="outline"
                    className={`w-full ${platform.borderColor} ${platform.textColor} hover:bg-${platform.color}/20 hover:text-white bg-transparent`}
                  >
                    ...
                    {isPlatformConnected(platform.id) ? `${platform.name} connecté ✓` : `Connecter ${platform.name}`}
                  </Button>
                ))}
              </CardContent>
            </Card>
